CREATE OR REPLACE  VIEW `v_bpm_todo_ru_task` AS  
SELECT
 `task`.`ID_`               AS `ID_`,
  `task`.`REV_`              AS `REV_`,
  ...
  `hi`.`CLAIM_TIME_`     AS `CLAIM_TIME_`,
  `hi`.`END_TIME_`       AS `END_TIME_`
FROM
  ACT_RU_TASK task,
  ACT_HI_TASKINST hi ,
  v_bpm_todo todo,
  bpm_approve app
WHERE hi.`PROC_INST_ID_` = task.`PROC_INST_ID_`
  AND hi.`TASK_DEF_KEY_` != task.`TASK_DEF_KEY_`
  AND todo.`instanceid` = task.`PROC_INST_ID_`
  AND todo.`tenantId` = task.`TENANT_ID_`
  AND app.`instanceid` = task.`PROC_INST_ID_`
